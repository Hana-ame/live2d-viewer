...
1. **选择 ZIP 压缩包**（推荐，手机也能用）——把模型文件夹压成 zip 后选它。
   - 压缩包里有顶层目录（如 `阿库露_vts/…`）会自动去掉，与文件夹选择等价。
   - 支持中文文件名；ZIP 需是标准 deflate 或无压缩（ZIP64 未支持）。
2. **选择模型文件夹**——选中模型目录（该目录需含 `.model3.json`）。
   - ⚠️ 依赖 `webkitdirectory`，**iOS Safari 等不支持**，此时请用 ZIP。
3. **选择文件（多选）**——把 `.model3.json` / `.moc3` / 贴图等**一起**选中。

文件夹或 ZIP 都可以**直接拖进左侧面板**。

加载后：

- **表情**按模型实际拥有的列出；有「模式（开关 / 直接设置）」与「叠加（不叠加 / 可叠加）」两组选项
- **动作**与表情一样会扫描目录，`model3.json` 未声明的 `*.exp3.json` / `*.motion3.json` 也能识别（这类"只放文件不写清单"的皮套很常见）
- 拖拽可动头部、点击可触发动作
...
**你的模型全程只在你自己的浏览器内存里，不经过任何服务器。**

```
你的磁盘（原文件不动）
   ↓ 你选择文件/文件夹/ZIP
浏览器内存
   ├─ File 对象（浏览器沙箱内，网页拿不到真实路径）
   └─ Blob URL（blob:https://… 形式的临时句柄，指向内存）
   ↓
渲染器 fetch(blobURL) → 画到 canvas
```

本站是**纯静态托管**（GitHub Pages），**没有任何后端接收接口**；全项目对 `localStorage` / `indexedDB` / `sessionStorage` / `Cache` **零使用**；关闭页面时 `beforeunload` 会逐个 `revokeObjectURL` 释放；再上传一个模型会先释放上一个。

**你可以自己验证**：按 `F12` → **Network（网络）** → 上传一个模型。你会看到**只有下载**（贴图等，来源是 `blob:`），**不会出现任何 POST / PUT 上传请求**。

- 上传文件夹与上传 ZIP，**存的地方完全一样**，只是读进来的方式不同（ZIP 在浏览器内解压）。
- 你的原文件**不会**被修改或移动。

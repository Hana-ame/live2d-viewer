...
/**
 * Cubism Core のバージョン差を吸収するヘルパ。
 *
 * Cubism Core 5.1 以降、`csmGetMocVersion` / `hasMocConsistency` は
 * バッファ長を第2引数に取るようになった。一方この Framework（5-r.5）は
 * 第1引数のみで呼ぶため、そのままでは
 *   TypeError: Cannot read properties of undefined (reading 'byteLength')
 * でモデルの読み込みに失敗する。
 *
 * 旧シグネチャ（1引数）と新シグネチャ（2引数）の両方で動くよう、
 * Core 側の関数長（length）を見て引数を足す。
 */
const coreMocVersion = (mocBytes: ArrayBuffer): number => {
  const fn = Live2DCubismCore.Version.csmGetMocVersion as (
    b: ArrayBuffer,
    n?: number
  ) => number;
  return fn.length >= 2 ? fn(mocBytes, mocBytes.byteLength) : fn(mocBytes);
};

const coreHasMocConsistency = (mocBytes: ArrayBuffer): number => {
  const fn = Live2DCubismCore.Moc.prototype.hasMocConsistency as (
    b: ArrayBuffer,
    n?: number
  ) => number;
  return fn.length >= 2 ? fn(mocBytes, mocBytes.byteLength) : fn(mocBytes);
...
export class CubismMoc {
  ...
  public static create(
    ...
    if (moc) {
      ...
      cubismMoc._mocVersion = coreMocVersion(mocBytes);
    }
  ...
  public static getMocVersionFromBuffer(mocBytes: ArrayBuffer): number {
    return coreMocVersion(mocBytes);
  }
  ...
  public static hasMocConsistency(mocBytes: ArrayBuffer): boolean {
    const isConsistent = coreHasMocConsistency(mocBytes);

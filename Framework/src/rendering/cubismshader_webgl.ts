...
export class CubismShader_WebGL {
  ...
  public constructor() {
    ...
    // 既定はページ直下からの相対パス。
    // 元の SDK サンプルは Samples/TypeScript/Demo から 2 階層上を指す
    // '../../Framework/...' を既定にしているが、本リポジトリはフラット構成で
    // シェーダを配信ルート直下に置くため './' 始まりにする。
    // こうしないと GitHub Pages のサブパス配信（/<repo>/）で
    // リポジトリ外を指してしまい、シェーダが読めず描画が空になる。
